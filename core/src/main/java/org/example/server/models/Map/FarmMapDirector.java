...
public class FarmMapDirector {
    public void buildMapWithoutForaging(FarmMapBuilder builder, int index) {
        ...
        if (index == 0) {
            ...
            ClientApp.getCurrentGame().getNpcMap().setPassageToFarmMap(6, 0, builder.getFinalProduct(), 54, 73);
        } else if (index == 1) {
            builder.createBigLake(25, 7);
            builder.setPassageToNpcValley(0, 74, 15, 1);
            ClientApp.getCurrentGame().getNpcMap().setPassageToFarmMap(15, 0, builder.getFinalProduct(), 54, 1);
            ...
            ClientApp.getCurrentGame().getNpcMap().setPassageToFarmMap(6, 33, builder.getFinalProduct(), 0, 73);
            ...
            ClientApp.getCurrentGame().getNpcMap().setPassageToFarmMap(15, 33, builder.getFinalProduct(), 0, 1);
        }
    }

    public void buildMap(FarmMapBuilder builder, int index) {
        buildMapWithoutForaging(builder, index);
        builder.generateForagingItems();
    }

    // TODO: rassa, bayad pak koni in payini ro va az balayee estefade beshe
    public void buildMapAshghal(FarmMapBuilder builder, int index) {
        builder.setDimensions(55, 75);
        builder.createHut();
        builder.createGreenHouse();
        builder.createStoneQuarry();
