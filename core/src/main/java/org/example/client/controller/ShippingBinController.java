...
public class ShippingBinController {
    ...
    public ShippingBinController(HUDView hudView, Stage stage) {
        ...
        okButton.setVisible(hudView.getCurrentMenu() == InGameMenuType.SHBIN);
...
//        stage.setDebugAll(true);
    ...
    public void displayFridge() {
        ...
        itemImages.forEach(imageButton -> {
            imageButton.setVisible(hudView.getCurrentMenu() == InGameMenuType.SHBIN);
        });
        quantityLabels.forEach(quantityLabel -> {
            quantityLabel.setVisible(hudView.getCurrentMenu() == InGameMenuType.SHBIN);
        });
        selectedItemImages.forEach(selectedItemImage -> {
            selectedItemImage.setVisible(hudView.getCurrentMenu() == InGameMenuType.SHBIN);
        });
        selectedQuantityLabels.forEach(selectedQuantityLabel -> {
            selectedQuantityLabel.setVisible(hudView.getCurrentMenu() == InGameMenuType.SHBIN);
        });

        if (hudView.getCurrentMenu() == InGameMenuType.SHBIN) {
            for (int i = 0; i < inventoryCopy.size(); i++) {
                ...
                ) && backpack.getItems().size() < 12) {
                    if (stacks.getItem() instanceof ToolType) {
                        error.getMessage().remove();
                        error = new GraphicalResult("The Selected Item cannot be Sold!", true);
                        ...
                        shippingBin.addItem(new Stacks(stacks.getItem(), stacks.getStackLevel(), 1));
                        backpack.addItems(stacks.getItem(), stacks.getStackLevel(), 1);
                        setupFridge();
                    }
                }
            }
...
//            inventoryCopy.add(new Stacks(item.getItem(), item.getStackLevel(), item.getQuantity()));
        ...
        for (Stacks stacks : inventoryCopy) {
            ...
            image.setVisible(hudView.getCurrentMenu() == InGameMenuType.SHBIN);
            ...
            label.setColor(com.badlogic.gdx.graphics.Color.RED);
            label.setFontScale(0.75f);
            label.setPosition(x + 40, y + 40);

            label.setVisible(hudView.getCurrentMenu() == InGameMenuType.SHBIN);
        ...
        selectedItems = backpack.getItems();
    ...
    public void setShippingBin(ShippingBin shippingBin) {
        ...
        hudView.setCurrentMenu(InGameMenuType.SHBIN);
        setupFridge();
    ...
    private void addListeners() {
        okButton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                error.getMessage().remove();
                error = new GraphicalResult("You got " + shippingBin.refresh() + "$ from what you sold!", false);
                error.setPosition((Gdx.graphics.getWidth() - error.getMessage().getWidth()) / 2f,
                        Gdx.graphics.getHeight() - 70);
                stage.addActor(error.getMessage());
                backpack.getItems().clear();
